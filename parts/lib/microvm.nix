{self}: let
  ...
  is-microvm-host = config: (has-microvm config) && builtins.hasAttr "vms" config.microvm;

  is-microvm-guest = config: (has-microvm config) && builtins.hasAttr "hypervisor" config.microvm;

  is-recursive-microvm = config: (is-microvm-host config) && (is-microvm-guest config);
  ...
  generate-tailscale-share = source: let
